...
6. Keep the deletion ledger until the post-prune audit proves the last backup dependency capable of restoring the deleted content is gone. On an approved deletion, use the same 21-day maximum for the live-store version/segment cleanup and for the backup dependency audit; do not wait for a nominal day-30 batch.
